...
- [x] 1.1 Generate a 2048-bit RSA key pair (no passphrase):
...
- [x] 1.2 Convert the public key to JWKS format (`smoke/dev-idp/jwks.json`). The JWKS must
...
- [x] 2.1 `smoke/dev-idp/README.md`: "These are THROWAWAY smoke-only keys. The private key
...
- [x] 2.2 Confirm `smoke/dev-idp/` is NOT in `.gitignore` (it must be committed).

## 3. Verification

- [x] 3.1 `jq . smoke/dev-idp/jwks.json` — valid JSON with `keys[0].kid`, `n`, `e`, `kty=RSA`, `alg=RS256`.
- [x] 3.2 Mint a test RS256 JWT with `node -e "..."` (using `jsonwebtoken` + the PEM private key)
