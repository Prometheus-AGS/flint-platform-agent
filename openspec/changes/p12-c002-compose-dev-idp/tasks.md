...
- [ ] 1.1 Add `dev-idp-jwks` service to both `compose.fabric.yml` and `compose.real.yml`:
...
- [ ] 2.1 `compose.fabric.yml`: update `fabric-gateway.GATEWAY_JWKS_URL` from the
  placeholder (`http://fabric-gateway:8080/.well-known/jwks.json`) to
  `http://dev-idp-jwks:8080/jwks.json`. Add `depends_on: dev-idp-jwks`.
- [ ] 2.2 `compose.real.yml`: update `fabric-gateway.GATEWAY_JWKS_URL` from
...
- [ ] 2.3 `compose.fabric.yml` + `compose.real.yml`: add port mappings to `fabric-keto`:
  `- "14466:4466"` (read) and `- "14467:4467"` (write). Both needed from the host for
  the Keto seed + health check.

## 3. Verification

- [ ] 3.1 `docker compose -f smoke/compose.fabric.yml config` — parses without error.
- [ ] 3.2 `docker compose -f smoke/compose.real.yml config` — parses without error.
- [ ] 3.3 Brief up test (just the JWKS server + `curl http://localhost:<host_port>/jwks.json`)
